# PyTorch Implementation of Adaptable ResNet Architecture based on: 
# https://github.com/pytorch/vision/blob/main/torchvision/models/resnet.py

# TODO: adaptableresnet should be adaptable by number of layers instead of model fraction for precision
# and experiment purposes. the model fraction feature can be added by holding a dictionary with
# keys being num layers and values being model fractions, and then u can adjust very simply
...
class AdaptableBasicBlock(nn.Module):
    expansion: int = 1
...
class AdaptableResNet(nn.Module):
    def __init__(
        self,
        block: Type[Union[AdaptableBasicBlock, Bottleneck]],
        layers: List[int],
        ...
        if zero_init_residual:
            for m in self.modules():
                ...
                elif isinstance(m, AdaptableBasicBlock) and m.bn2.weight is not None:
    ...
    def reconfigure(self, model_fraction: float) -> None:
        ...
        # deactivates all blocks
        self.deactivate()
        num_active_params = self.min_num_params
        # NOTE: Decreasing the number of parameters in further layers might be beneficial.
        for layer in self.layers:
            for block in layer:
                if (num_active_params + block.adaptable_parameters) <= model_fraction * self.num_parameters:
