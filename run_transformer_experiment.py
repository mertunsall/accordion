...
import utils, train_transformer
from fed_trainer_transformer import AdaptableFedTrainer
...
def run_exp(config):
    ...
    block_probabilities = config['block_probabilities']
    ...
    if exp_type != "FedClass":
        ...
        for i in range(n_models):
            global_models.append(AdaptableVisionTransformer(patch_size, max_len, embed_dim, num_classes, layers, channels, heads).to(device))
    ...
    if "Centralized" in exp_type:
        ...
        if exp_type in ["Centralized", "CentralizedSmall"]:
            training_loss, val_loss, val_acc, lr_rates = train_transformer.train_and_save(global_model, 
                device, SAVE_PATH, val_size=val_size, train_size=train_size, lr = lr,
                n_epochs=global_epochs, dataset=dataset)

        else:
            print(f"Block Probabilities: {block_probabilities}")
            training_loss, val_loss, val_acc, lr_rates = train_transformer.train_and_save(global_model, 
                device, SAVE_PATH, train_size=train_size, val_size=val_size, block_probabilities=block_probabilities,
                distillation=distillation, lr = lr, n_epochs=global_epochs, 
                dataset=dataset, beta = beta)

        _, test_accuracy = train_transformer.loss_and_accuracy(global_model, testloader, nn.CrossEntropyLoss(), device)
        ...
        if exp_type == 'FedClass':
            ...
            for i in range(n_models):
                ...
                adaptable_trainers.append(AdaptableFedTrainer(global_models[i], tset, vset, 
                        ...
                        global_epochs, lr = lr, local_epochs = local_epochs, 
            ...
            for i in range(n_models):
                _, test_accuracy = train_transformer.loss_and_accuracy(global_models[i], testloader, nn.CrossEntropyLoss(), device)
            ...
            adaptable_trainer = AdaptableFedTrainer(global_model, trainset, valset, n_devices, num_blocks,
                n_device_per_round, device, global_epochs, local_epochs = local_epochs, lr = lr,
