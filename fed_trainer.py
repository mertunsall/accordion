...
class AdaptableFedTrainer:
    ...
    def __init__(self, global_model, trainset, valset, n_devices, num_blocks, n_device_per_round, device, 
        ...
        self.fed_dataloader = FedDataLoader(n_devices, self.fed_dataset, seed = self.seed)   
        ...
        for epoch in range(self.local_epochs):
            ...
            for i, data in enumerate(trainloader, 0):
                ...
                if distill and local_model.active_blocks != self.device_num_blocks[-1]:
                    ...
                    outputs, curr_activation = train.forward_and_final_activation(local_model, inputs, eval = False) 

                    local_model.reconfigure_blocks(next_device_blocks)
                    _, next_activation =  train.forward_and_final_activation(local_model, inputs, eval = False) 
    ...
    def get_device_accuracies(self, num_blocks):
        ...
        for i in num_blocks:
            ...
            if i in acc_dict:
                ...
                _, test_accuracy = train.loss_and_accuracy(self.global_model, self.testloader, self.criterion, self.device)
